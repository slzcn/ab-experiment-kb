...
async function boot(){
  ...
  loadSearchIndex();
  // 首屏出来后，浏览器空闲时只预热【当前可见那几篇】的正文(单篇 d/<id>.json)，
  // 不一把拉 2.1MB 全量，省流量。用户每往下滞一批再预热那一批(见 _appendBatch)。
  const _pre=()=>prefetchVisible();
  if('requestIdleCallback' in window) requestIdleCallback(_pre, {timeout:3000});
  else setTimeout(_pre, 1200);
}
...
function _appendBatch(){
  ...
  // 本批卡片出现后，空闲时预热这一批的正文（每往下滞一批就提前拉那一批，省流量）
  if('requestIdleCallback' in window) requestIdleCallback(()=>prefetchVisible(), {timeout:3000});
  else setTimeout(()=>prefetchVisible(), 300);
}
...
// 【按需/分片预热】DOCS_CACHE 按 id 存单篇正文。
// 预热不拉整份 2.1MB 的 kb_docs.json，而是只拉可见那几篇的单篇 d/<id>.json(每篇仅几KB)，
// 省流量；用户每往下滞一批就预热那一批。全量 kb_docs.json 仅作最后兽底。
let DOCS_CACHE={}, _docsFullInflight=null;
const _docInflight={};   // id -> Promise，防单篇并发重复拉
// 拉单篇正文 d/<id>.json 存入 DOCS_CACHE（已有/已在拉则直接复用）
function prefetchDoc(id){
  const key=String(id);
  if(DOCS_CACHE[key]!=null) return Promise.resolve(DOCS_CACHE[key]);
  if(_docInflight[key]) return _docInflight[key];
  const d=KB.docs&&KB.docs.find(x=>x.id===+id);
  if(d && d.md){ DOCS_CACHE[key]=d.md; return Promise.resolve(d.md); }  // 已有正文(直达 INIT_MD/内嵌)
  _docInflight[key]=(async()=>{
    try{
      const r=await fetchT(dataUrl('d/'+key+'.json'), 12000);
      if(r.ok){ const j=await r.json(); if(j && typeof j.md==='string'){ DOCS_CACHE[key]=j.md; } }
    }catch(e){}
    delete _docInflight[key];
    return DOCS_CACHE[key];
  })();
  return _docInflight[key];
}
// 兽底：拉全量 kb_docs.json(单篇拉失败时才用)
function ensureDocsCache(){
  if(_docsFullInflight) return _docsFullInflight;
  _docsFullInflight=(async()=>{
    try{ const r=await fetchT(dataUrl('kb_docs.json'), 20000); if(r.ok){ const all=await r.json(); Object.assign(DOCS_CACHE, all); } }
    catch(e){}
    _docsFullInflight=null;
    return DOCS_CACHE;
  })();
  return _docsFullInflight;
}
// 预热当前列表已渲染(可见)的那批文章正文。idle 时逐篇拉单篇，不抢带宽。
function prefetchVisible(){
  try{
    const ids=RES.slice(0, SHOWN).map(d=>d.id).filter(id=>DOCS_CACHE[String(id)]==null);
    ids.forEach(id=>prefetchDoc(id));
  }catch(e){}
}
async function getMd(id){
  const key=String(id);
  const d=KB.docs.find(x=>x.id===id);
  if(d && d.md) return d.md;                    // 已有正文（内嵌/直达 INIT_MD）
  if(DOCS_CACHE[key]!=null) return DOCS_CACHE[key]; // 已预热
  const one=await prefetchDoc(id);              // 未预热到→单篇拉
  if(one!=null) return one;
  await ensureDocsCache();                      // 单篇拉不到→兽底拉全量
  return (DOCS_CACHE[key]!=null)?DOCS_CACHE[key]:'（正文加载失败，请刷新重试）';
}
